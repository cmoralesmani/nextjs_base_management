    ...
    try {
      ...
      if (hasPermissionToShowUsers) {
        if (data.users_selected?.length) {
          const listUsersPermission = data.users_selected.map((p) => ({
            CREATED_AT: moment().format('YYYY-MM-DD HH:mm:ss'),
            CREATED_BY: req.user.username,
            CREATED_IN: 'API_WEB_TP',
            MODIFIED_AT: moment().format('YYYY-MM-DD HH:mm:ss'),
          ...
          await db.bmauth_user_profiles.bulkCreate(listUsersPermission, {
            transaction
          })
        }
      }
      ...
      if (hasPermissionToShowPermissions) {
        if (data.permissions_selected?.length) {
          const listProfilePermission = data.permissions_selected.map(
            (p) => ({
              CREATED_AT: moment().format('YYYY-MM-DD HH:mm:ss'),
              CREATED_BY: req.user.username,
              CREATED_IN: 'API_WEB_TP',
