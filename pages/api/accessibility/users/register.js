  ...
  validateMiddleware(
    [
      check('username').custom(async (username) => {
        ...
        if (!validausuario.test(username)) { throw new Error('El nombre de usuario no valido') }
  ...
  async function post () {
    ...
    try {
      ...
      if (hasPermissionToShowProfiles && data.profiles_selected?.length) {
        const listUserProfile = data.profiles_selected.map((p) => ({
          CREATED_AT: moment().format('YYYY-MM-DD HH:mm:ss'),
          CREATED_BY: req.user.username,
          CREATED_IN: 'API_WEB_TP',
          MODIFIED_AT: moment().format('YYYY-MM-DD HH:mm:ss'),
      ...
      if (
        ...
        const listUserPermission = data.permissions_selected.map((p) => ({
          CREATED_AT: moment().format('YYYY-MM-DD HH:mm:ss'),
          CREATED_BY: req.user.username,
          CREATED_IN: 'API_WEB_TP',
          MODIFIED_AT: moment().format('YYYY-MM-DD HH:mm:ss'),
        ...
        await db.bmauth_user_permissions.bulkCreate(listUserPermission, {
          transaction
        })
      }
